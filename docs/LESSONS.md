...
| L24 | QA crashed headless Chrome from 0.7 on | 0.7 moved the game to its own clock, but the QA bot still asked the rules using `performance.now()`. The bot and the game disagreed ("scrape" versus "grab"), and thousands of rejected actions each spawned a floating error text | When a shared concept changes (clock, units, ids), grep **every** consumer, QA and tests included, in the same build. |
| L25 | A10 (fast-forward) measured 0 robot bricks at both ×1 and ×4 | CI's software GL runs about 3 fps and every frame is capped at 50 ms of game time, so 3 real seconds ≈ 0.5 game seconds | Time-based checks measure **game-clock time per frame**, never real seconds. |
